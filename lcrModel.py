...
def main(train_path, test_path, accuracyOnt, test_size, remaining_size, learning_rate=FLAGS.learning_rate, keep_prob=FLAGS.keep_prob1, momentum=0.95, l2=0.00001):
    ...
    with tf.Session(config=config) as sess:
        ...
        fp = open(FLAGS.prob_file, 'a')
        for k, v in sorted(FLAGS.flag_values_dict().items()):
            fp.write('parameters used in this run')
            fp.write('{}={}'.format(k, v))
        fp.write("\n")
        fp.write("results of the run: in_sample = {}, out_of_sample = {}, ontology_acuracy = {}, total_acc = {}".format(max_trainacc, max_acc, accuracyOnt, max_totalacc))
        fp.write("\n")
        fp.write("at iteration {}".format(iteration))
